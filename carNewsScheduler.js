...
class CarNewsScheduler {
  constructor() {
    this.isRunning = false;
    this.intervalId = null;
  }

  // Start continuous car news fetching
  async startScheduler(intervalMinutes = 30) {
    if (this.isRunning) {
      console.log('Car Scheduler already running');
      return;
    }

    this.isRunning = true;
    console.log(`Starting car news scheduler - fetching every ${intervalMinutes} minutes`);
  ...
  stopScheduler() {
    ...
    this.isRunning = false;
    console.log('Car news scheduler stopped');
  }

  // Fetch car news from GNews API
  async fetchNewsFromAPI() {
    try {
      ...
      console.error('Error fetching car news from API:', error);
      return [];
    }
  }

  // Store car news in database
    ...
    try {
      for (const article of articles) {
        ...
        const [existing] = await pollDBPool.query(
          'SELECT id FROM car_news_openai WHERE title = ? AND source_url = ?',
        ...
        if (existing.length === 0) {
          ...
          console.log(`Stored Car: ${article.title.substring(0, 50)}...`);
        }
      }
    } catch (error) {
      console.error('Error storing car news in database:', error);
    }
  }

  // Main function to fetch and store car news
  async fetchAndStoreNews() {
    console.log('Fetching latest car news...');
    const articles = await this.fetchNewsFromAPI();
    await this.storeNewsInDB(articles);
    console.log(`Fetched and stored ${articles.length} car articles`);
  }

  // Get stored car news from database
  async getStoredNews(limit = 100, offset = 0) {
    try {
      ...
      console.error('Error getting stored car news:', error);
      return [];
    }
  }

  // Mark car article as processed
  async markAsProcessed(articleId, readyArticle) {
    try {
      await pollDBPool.query(
        'UPDATE car_news_openai SET openai_processed = true, processed_at = NOW(), openai_ready_article = ? WHERE id = ?',
        [readyArticle, articleId]
      );
    } catch (error) {
      console.error('Error marking car article as processed:', error);
    }
  }
}

module.exports = CarNewsScheduler;
