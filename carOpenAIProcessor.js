  ...
  try {
    ...
    const response = await axios.post(OPENAI_BASE_URL, {
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert car journalist and SEO editor. Write engaging, comprehensive car content in English with deep knowledge of cars, automotive industry, technology, and automotive culture. Always provide detailed, accurate automotive analysis and compelling storytelling in English."
        },
        {
          role: "user",
          content: prompt
...
function buildCarPrePublishPrompt({ title, description, body }) {
  return `
You are an expert car journalist and SEO editor. Analyze this car news and provide comprehensive SEO recommendations.
...
Advanced SEO rules for car news:
...
- Make headlines attractive and car-specific (include car model, price range, key features)
- Headlines must include car model, brand name, price, or main event
- Meta description should highlight main car story and create curiosity
...
function buildCarRewriteBodyHtmlPrompt({ title, description, body, recommendations }) {
  return `
You are an expert car journalist. Rewrite this car news article into a comprehensive, engaging HTML article.
...
async function processCarNewsOpenAI(articleData, options = {}) {
  try {
    console.log('🚗 Starting car news OpenAI processing...');
    
    const { title, description, content } = articleData;
    
    // Step 1: Get SEO recommendations
    console.log('📊 Getting SEO recommendations...');
    const prePrompt = buildCarPrePublishPrompt({
      title,
      description,
      body: content
    });
    ...
    const htmlPrompt = buildCarRewriteBodyHtmlPrompt({
      title,
      description,
      body: content,
    ...
    console.error('❌ Car news processing error:', error);
    return {
      success: false,
      error: error.message
    };
...
module.exports = {
  processCarNewsOpenAI,
  buildCarPrePublishPrompt,
  buildCarRewriteBodyHtmlPrompt
};
