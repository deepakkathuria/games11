...
/**
 * Fetch all news from GNews API
 */
async function fetchAllNews(options = {}) {
  try {
    const {
      query = "news", // Changed from cricket to news
      lang = "en",
      country = "in",
      max = 25,
      expand = "content"
 ...
 */
function filterAllNewsArticles(articles, filters = {}) {
  const { 
 ...
 */
function getAllNewsArticleSummary(article) {
 ...
 */
function validateAllNewsArticleForProcessing(article) {
  const errors = [];
...
module.exports = {
  fetchAllNews,
  filterAllNewsArticles,
  getAllNewsArticleSummary,
  validateAllNewsArticleForProcessing
};